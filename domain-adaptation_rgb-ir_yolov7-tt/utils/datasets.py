...
from utils.general import check_requirements, xyxy2xywh, xywh2xyxy, xywhn2xyxy, xyn2xy, xyxyn2xyxy, segment2box, segments2boxes, \
...
def img2label_paths(img_paths):
    # Define label paths as a function of image paths
    return [x.split(' ')[0] for x in img_paths]
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            self.img_files = sorted([x.replace('/', os.sep) for x in f if x.split(' ')[0].split('.')[-1].lower() in img_formats])
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    l = np.array([list(map(int, m.split(','))) for m in im_file.split(' ')[1:]], dtype=np.float32)
                    l = np.unique(l, axis=0)
                    l[:, [0, 2]] = l[:, [0, 2]]/shape[0]
                    l[:, [1, 3]] = l[:, [1, 3]]/shape[1]
                    classes = l[:,[4]]
                    segments = l[:,:4]
                    l = np.concatenate((classes, segments), 1)
                    if len(l):
                    ...
                    nm += 1  # label missing
                    l = np.zeros((0, 5), dtype=np.float32)
                x[im_file.split(' ')[0]] = [l, shape, [si for si in segments.reshape(-1,2,2)]]
            except Exception as e:
                nc += 1
        ...
        # x['hash'] = get_hash(self.label_files)
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if labels.size:
            labels[:, 1:] = xyxyn2xyxy(labels[:, 1:], w, h, padw, padh)  # normalized xyxy to pixel xyxy format
...
def load_mosaic9(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        labels, segments = self.labels[index].copy(), self.segments[index].copy()
        if labels.size:
            labels[:, 1:] = xyxyn2xyxy(labels[:, 1:], w, h, padx, pady)  # normalized xyxy to pixel xyxy format
